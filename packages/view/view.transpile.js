    ...
    run() {
        ...
        if (useTailwind) {
            ...
            if (fs.existsSync(inputCSSFile)) {
                (0, node_child_process_1.execSync)(`npx tailwindcss -i ${inputCSSFile} -o ${outputCSSTailwind} --config ${tailwindConfigPath} --minify`, { stdio: 'ignore' });
                this.logger.log('Tailwind CSS compiled successfully');
            }
            else if (fs.existsSync(inputCSSFileSrc)) {
                (0, node_child_process_1.execSync)(`npx tailwindcss -i ${inputCSSFileSrc} -o ${outputCSSTailwind} --config ${tailwindConfigPath} --minify`, { stdio: 'ignore' });
                this.logger.log('Tailwind CSS compiled successfully');
            }
            else {
