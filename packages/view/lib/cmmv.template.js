...
class Template {
    ...
    async loadIncludes(templateText) {
        ...
        while ((match = includeRegex.exec(templateText)) !== null) {
            ...
            const resolvedPath = path.resolve((0, process_1.cwd)(), includePath);
            const resolvedPathSimplifly = path.resolve((0, process_1.cwd)(), 'public', 'views', includePath);
    ...
    async extractInlineScripts(html) {
        ...
        let resultHtml = html;
        let scriptIndex = 0;
        const tempDir = path.resolve((0, process_1.cwd)(), './public/assets');
        ...
        while ((match = scriptRegex.exec(html)) !== null) {
            ...
            resultHtml = resultHtml.replace(match[0], `<script src="${tempFilePath.replace((0, process_1.cwd)(), '').replace('/public', '')}" nonce="${this.nonce}"></script>`);
        }
        this.parseScripts();
    ...
    async processSetup(result) {
        ...
        if (result.setup) {
            ...
            if (templateCache[cacheKey]) {
                ...
                const file = path.resolve((0, process_1.cwd)(), `./public/templates/${result.setup.layout}.html`);
                const fileSubdir = path.resolve((0, process_1.cwd)(), `./public/templates/**/${result.setup.layout}.html`);
