...
export class GlobalProto extends Singleton {
    ...
    static async load() {
        let directoryPackages = path.resolve((process.env.NODE_ENV === "prod") ? "./node_modules/@cmmv/**/*.proto" : "./packages/**/*.proto");
        let directory = path.resolve((process.env.NODE_ENV === "prod") ? "./dist/**/*.proto" : "./src/**/*.proto");

        const files = await fg([directoryPackages, directory], { ignore: ["node_modules/**"] });
    ...
    static retrieveTypes(key: string | number, message: string | number): string | null {
        ...
        const Types = (typeof key === "string" && globalProto.types.has(key)) ? globalProto.types.get(key) : null;
