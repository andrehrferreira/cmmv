...
class AuthTranspile {
    ...
    run() {
        ...
        if (hasWs)
            this.generateWebSocketIntegration();
    }
    async generateController() {
        ...
        const hasRepository = core_1.Module.hasModule('repository');
...
export class AuthController {
    ...
    async register(@Body() payload: RegisterRequest): Promise<RegisterResponse> {
        ...
        core_1.Application.appModule.controllers.push({
            name: 'AuthController',
    ...
    async generateService() {
        ...
        const hasRepository = core_1.Module.hasModule('repository');
        const hasCache = core_1.Module.hasModule('cache');
...
export class AuthService extends AbstractService {
    public async login(
        ...
        const token = jwt.sign({ 
            id: ${core_1.Config.get('repository.type') === 'mongodb' ? `user._id` : `user.id`},
        ...
        ${hasCache ? `CacheService.set(\`user:\${${core_1.Config.get('repository.type') === 'mongodb' ? `user._id` : `user.id`}}\`, JSON.stringify(user), expiresIn);\n` : ''}        
