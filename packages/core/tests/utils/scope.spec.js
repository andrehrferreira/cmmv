...
const scope_utils_1 = require("../../utils/scope.utils");
describe('Scope', function () {
    beforeEach(function () {
        // Clear any existing data before each test to ensure isolation
        scope_utils_1.Scope.clear('testData');
        scope_utils_1.Scope.clear('testArray');
    });
    it('should set and retrieve data by name', function () {
        const result = scope_utils_1.Scope.set('testData', { key: 'value' });
        assert_1.strict.strictEqual(result, true);
        const data = scope_utils_1.Scope.get('testData');
        assert_1.strict.deepStrictEqual(data, { key: 'value' });
    });
    it('should not overwrite existing data', function () {
        scope_utils_1.Scope.set('testData', { key: 'value1' });
        const result = scope_utils_1.Scope.set('testData', { key: 'value2' });
    ...
    it('should return true if data exists', function () {
        scope_utils_1.Scope.set('testData', { key: 'value' });
        assert_1.strict.strictEqual(scope_utils_1.Scope.has('testData'), true);
    });
    it('should return false if data does not exist', function () {
        assert_1.strict.strictEqual(scope_utils_1.Scope.has('nonExistentData'), false);
    });
    it('should return null for non-existent data', function () {
        const data = scope_utils_1.Scope.get('nonExistentData');
    ...
    it('should add to an array in scope', function () {
        const result1 = scope_utils_1.Scope.addToArray('testArray', 'value1');
        const result2 = scope_utils_1.Scope.addToArray('testArray', 'value2');
        assert_1.strict.strictEqual(result1, true);
        assert_1.strict.strictEqual(result2, true);
        const array = scope_utils_1.Scope.getArray('testArray');
        assert_1.strict.deepStrictEqual(array, ['value1', 'value2']);
    });
    it('should not add to non-array data', function () {
        scope_utils_1.Scope.set('testData', { key: 'value' });
        const result = scope_utils_1.Scope.addToArray('testData', 'value2');
        assert_1.strict.strictEqual(result, false);
    });
    it('should remove from an array in scope', function () {
        scope_utils_1.Scope.addToArray('testArray', 'value1');
        scope_utils_1.Scope.addToArray('testArray', 'value2');
        const result = scope_utils_1.Scope.removeFromArray('testArray', 'value1');
        assert_1.strict.strictEqual(result, true);
        const array = scope_utils_1.Scope.getArray('testArray');
    ...
    it('should not remove from non-array data', function () {
        scope_utils_1.Scope.set('testData', { key: 'value' });
        const result = scope_utils_1.Scope.removeFromArray('testData', 'value2');
        assert_1.strict.strictEqual(result, false);
    });
