...
class ApplicationTranspile {
    ...
    generateModel(contract) {
        ...
        let includeId = '';
        if (modelInterfaceName !== 'IWsCall' &&
            modelInterfaceName !== 'IWsError')
            includeId = `${lib_1.Config.get('repository.type') === 'mongodb' ? '    _id?: ObjectId' : '    id?: any'};\n`;
        const modelTemplate = `// Generated automatically by CMMV

${this.generateClassImports(contract)}
        
export interface ${modelInterfaceName} {
${includeId}${contract.fields?.map((field) => `    ${field.propertyKey}: ${this.mapToTsType(field.protoType)};`).join('\n')}
}
...
`;
    ...
    generateClassImports(contract) {
        ...
        if (lib_1.Config.get('repository.type') === 'mongodb') {
        ...
        const hasType = contract.fields?.some((field) => field.protoType === 'date');
