...
export const GENERATE_CONTROLLER_METADATA = Symbol("generate_controller_metadata");
export const AUTH_METADATA = Symbol("auth_metadata");
export const CONTROLLER_CUSTOM_PATH_METADATA = Symbol("controller_custom_path_metadata");
...
export function Contract(options?: ContractOptions): ClassDecorator {
    ...
    return (target: object) => {
        ...
        Reflect.defineMetadata(GENERATE_CONTROLLER_METADATA, generateController, target);
        Reflect.defineMetadata(AUTH_METADATA, auth, target);
        Reflect.defineMetadata(CONTROLLER_CUSTOM_PATH_METADATA, controllerCustomPath, target);
    };
}
