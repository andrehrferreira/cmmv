...
export class RPCRegistry {
    private static controllers = new Map<any, { contract: string, messages: any[] }>();
    ...
    public static registerController(target: any, contract: string) {
        if (!this.controllers.has(target)) 
            ...
            this.controllers.set(target, { ...this.controllers.get(target), contract });
    }

    public static registerMessageHandler(target: any, message: string, handlerName: string) {
    ...
    public static clear(){
        RPCRegistry.controllers = new Map<any, { contract: string, messages: any[] }>();
    }
}
