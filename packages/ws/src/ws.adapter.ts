...
import { 
    AbstractHttpAdapter, AbstractWSAdapter, 
    Application, Logger 
} from "@cmmv/core";
...
export class WSAdapter extends AbstractWSAdapter {
    ...
    private registeredMessages: Map<string, { instance: any, handlerName: string, params: any[] }> = new Map();

    create(server: AbstractHttpAdapter, application: Application, options?: any) {
    ...
    private registerGateways() {
        ...
        controllers.forEach(([controllerClass, metadata]) => {
            ...
            metadata.messages.forEach((messageMetadata: any) => {
                ...
                this.registeredMessages.set(message, { instance, handlerName, params });
            });
        });
    }
}