...
export class RpcUtils {
    ...
    public static async pack(contractName: string, messageName: string, data?: any): Promise<Uint8Array | null> {
        return new Promise((resolve, reject) => {
            try{
                ...
                if(Number.isInteger(index) && contractRoot){                    
                    ...
                    const typeName = ProtoRegistry.retrieveTypes(contractName, messageName);  
                    const dataBuffer = (contract) ? contract.encode(data).finish() : null;

                    const buffer = ProtoRegistry.retrieve("ws").lookupType("WsCall").encode({
                        contract: index,
                        message: typeName,
