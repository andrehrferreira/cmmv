...
function createMessageDecorator(message: string): MethodDecorator {
    return (target, propertyKey: string | symbol) => {
        RPCRegistry.registerMessageHandler(target, message, propertyKey as string);
    };
}
...
export function Data(): ParameterDecorator {
    return (target, propertyKey: string | symbol, parameterIndex: number) => {
        RPCRegistry.registerParam(target, propertyKey as string, 'data', parameterIndex);
    };
}

export function Socket(): ParameterDecorator {
    return (target, propertyKey: string | symbol, parameterIndex: number) => {
        RPCRegistry.registerParam(target, propertyKey as string, 'socket', parameterIndex);
    };
}