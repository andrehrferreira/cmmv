...
function Controller(prefix = '') {
    return (target) => {
        ...
        controller_registry_utils_1.ControllerRegistry.registerController(target, prefix);
    };
}
function createMethodDecorator(method, path, cb) {
    return (target, propertyKey, context) => {
        controller_registry_utils_1.ControllerRegistry.registerRoute(target, method, path, propertyKey, context.value, cb);
    };
}
...
function createParamDecorator(paramType) {
    return (target, propertyKey, parameterIndex) => {
        const paramName = paramType.startsWith('param') ||
            ...
            ? paramType.split(':')[1]
            : paramType;
        controller_registry_utils_1.ControllerRegistry.registerParam(target, propertyKey, paramType, parameterIndex, paramName);
    };
}
function Body() {
