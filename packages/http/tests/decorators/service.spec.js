...
describe('Service Decorator', function () {
    ...
    it('should register a service with a given name', function () {
        ...
        const service = service_registry_util_1.ServiceRegistry.getService('MyService');
        assert_1.strict.strictEqual(service?.constructor, MyService);
    });
    it('should register a service with a default name when none is provided', function () {
        ...
        const service = service_registry_util_1.ServiceRegistry.getService('');
        assert_1.strict.strictEqual(service?.constructor, MyDefaultService);
    });
    ...
    it('should handle multiple services with different names', function () {
        ...
        const serviceOne = service_registry_util_1.ServiceRegistry.getService('ServiceOne');
        const serviceTwo = service_registry_util_1.ServiceRegistry.getService('ServiceTwo');
        assert_1.strict.strictEqual(serviceOne?.constructor, ServiceOne);
        assert_1.strict.strictEqual(serviceTwo?.constructor, ServiceTwo);
    });
    it('should replace existing service with the same name', function () {
        let FirstService = class FirstService {
        };
        FirstService = tslib_1.__decorate([
            (0, service_decorator_1.Service)('ReplaceService')
        ], FirstService);
        let SecondService = class SecondService {
        };
        SecondService = tslib_1.__decorate([
            (0, service_decorator_1.Service)('ReplaceService')
        ], SecondService);
        const service = service_registry_util_1.ServiceRegistry.getService('ReplaceService');
        assert_1.strict.strictEqual(service?.constructor, SecondService);
    });
    afterEach(function () {
        service_registry_util_1.ServiceRegistry.clear();
    });
